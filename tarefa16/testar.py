...
class BaseTask:
    ...
    def has_n_words(self, file_name, n_words):
        ...
        assert len(text.split()) >= n_words, f"{file_name} deve ter pelos menos {n_words} palavras"


class Task(BaseTask):
    def teste_0_grafo(self):
        arquivo = "grafo.md"
        self.exists(arquivo)
        self.has_n_words(arquivo, 60)

    def teste_1_pequeno(self):
        binary = self.compile(binary="rpg")
        self.test_cases(binary, ["exemplo.in", "pequeno1.in", "pequeno2.in", "pequeno3.in"])

    def teste_2_medio(self):
        binary = self.compile(binary="rpg")
        self.test_cases(binary, ["medio1.in", "medio2.in", "medio3.in"])

    def teste_3_grande(self):
        binary = self.compile(binary="rpg")
        self.test_cases(binary, ["grande1.in", "grande2.in", "grande3.in"])
