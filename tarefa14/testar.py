...
class Task(BaseTask):
    def teste_1_somente_enfileira(self):
        binary = self.compile(binary="roteador")
        self.test_cases(binary, self.case_range("0{}_only_enqueue.in", 1, 2))

    def teste_2_remove_um(self):
        binary = self.compile(binary="roteador")
        self.test_cases(binary, ['03_one_pop.in'])

    def teste_3_capacidade_pequena(self):
        binary = self.compile(binary="roteador")
        self.test_cases(binary, ['04_small_capacity.in'])

    def teste_4_remove_varios(self):
        binary = self.compile(binary="roteador")
        self.test_cases(binary, ['05_big_pop.in'])

    def teste_5_todas_operacoes(self):
        binary = self.compile(binary="roteador")
        self.test_cases(binary,
                self.case_range("0{}_generic.in", 6, 8))
