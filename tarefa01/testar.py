...
class Task(BaseTask):
    def teste_1_inverso(self):
        binary = self.compile(["rearranjar.c"])
        self.test_cases(binary, self.case_range("inverso_{}.in", 1, 3))

    def teste_2_espera(self):
        binary = self.compile(["rearranjar.c"])
        self.test_cases(binary, self.case_range("espera_{}.in", 1, 3))

    def teste_3_ambos(self):
        binary = self.compile(["rearranjar.c"])
        self.test_cases(binary, ["exemplo.in"])
        self.test_cases(binary, self.case_range("ambos_{}.in", 1, 3))

    def teste_4_borda(self):
        binary = self.compile(["rearranjar.c"])
        self.test_cases(binary, self.case_range("borda_{}.in", 1, 4))
