...
class Task(BaseTask):
    def teste_1_mini(self):
        binary = self.compile(["arranhaceu.c"])
        self.test_cases(binary, ["n_2A.in", "n_3A.in", "n_3B.in"])

    def teste_2_pequeno(self):
        binary = self.compile(["arranhaceu.c"])
        self.test_cases(binary, ["exemplo.in", "n_4A.in", "n_4B.in"])

    def teste_3_medio(self):
        binary = self.compile(["arranhaceu.c"])
        self.test_cases(binary, ["n_5A.in"])

    def teste_4_grande(self):
        binary = self.compile(["arranhaceu.c"])
        self.test_cases(binary, ["n_6A.in"])
