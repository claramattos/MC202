...
class Task(BaseTask):
    SOURCES = ["planejar.c", "main.c"]

    def teste_1_registrar(self):
        binary = self.compile(self.SOURCES)
        self.test_cases(binary, self.case_range("registrar{}.in", 1, 4))

    def teste_2_alteracoes_cancelamento(self):
        binary = self.compile(self.SOURCES)
        self.test_cases(binary, self.case_range("alteracoes_cancelamento{}.in", 1, 4))

    def teste_3_datas(self):
        binary = self.compile(self.SOURCES)
        self.test_cases(binary, ["exemplo.in"])
        self.test_cases(binary, self.case_range("datas{}.in", 1, 4))
