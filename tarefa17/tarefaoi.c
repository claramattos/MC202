...
int main(){
    ...
    p_no novo2;
    novo2 = malloc(sizeof(No));
    novo2->prox = NULL;
    novo2->j=j;
    novo2->valor_parcela = s;
    novo2->n_parcela = 1;
    novo2->qtd_parcelas_faltantes = n-1;
    
    if (novo2->valor_parcela == (int)novo2->valor_parcela) 
        printf("%.0f ", novo2->valor_parcela);
    else
        printf("%.2f ", novo2->valor_parcela);
    ...
    while (tudo_calculado != 0) {
        ...
        for (i=0;i<100;i++) {
            if (vetor[i] == NULL)
                break;
            else if (vetor[i]->qtd_parcelas_faltantes>0) {
