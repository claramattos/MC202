...
class Task(BaseTask):
    def teste_1_mini(self):
        binary = self.compile(binary="nuvem")
        self.test_cases(binary, ["1_RitaOra.in", "6_CarlaBruni.in","11_LadyGaga.in"])

    def teste_2_pequeno(self):
        binary = self.compile(binary="nuvem")
        self.test_cases(binary, ["16_Beyonce.in","21_Madonna.in","28_MichaelJackson.in"])

    def teste_3_medio(self):
        binary = self.compile(binary="nuvem")
        self.test_cases(binary, ["35_ArianaGrande.in","42_Adele.in","49_BillieEilish.in"])

    def teste_4_grande(self):
        binary = self.compile(binary="nuvem")
        self.test_cases(binary, ["56_TaylorSwift.in","71_KatyPerry.in","86_Anitta.in","101_Shakira.in"])
